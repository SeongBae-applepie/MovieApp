...
app.post("/insert_post", function (req, res) {
  ...
  const conn = mysql.createConnection(dbconfig);
  conn.connect();
  var sql = `INSERT INTO users_post (post_content, post_debate , post_movie_id, post_movie_name, post_title, uuid_users, post_views) VALUE ("${post_content}","${post_debate}","${post_movie_id}","${post_movie_name}","${post_title}","${uuid_users}",0)`;
